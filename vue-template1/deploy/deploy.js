...
function uploadDeploy (userName, password, serverAddr, serverPort) {
  ...
  exec(`"${defOpts.winSCP}" /console /command "option confirm off" "open ${defOpts.type}://${userName}:${password}@${serverAddr}:${serverPort}" "option transfer binary" "cd /home" "call if [ ! -d /home/deploy ];then mkdir /home/deploy; fi" "call if [ ! -d ${defOpts.uploadPath}/${pack.name} ];then mkdir ${defOpts.uploadPath}/${pack.name}; fi" "cd ${defOpts.uploadPath}/${pack.name}" "put dist.zip" "call rm -rf ${defOpts.websitePath}" "call unzip dist.zip -d ${defOpts.websitePath}" "call if [ ! -d ${defOpts.uploadPath}/${pack.name}/history ];then mkdir ${defOpts.uploadPath}/${pack.name}/history; fi" "call mv dist.zip ${defOpts.uploadPath}/${pack.name}/history/${pack.name}_${pack.version}_${datetime}.zip" "exit" /log=${defOpts.log}`, (error, stdout, stderr) => {
