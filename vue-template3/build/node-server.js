...
const rootDir = `dist/${multiConfig.process.name}`
...
server.listen(PORT);
console.log(chalk.green(`[${pack.name}_${multiConfig.process.name}] 生产包启动成功!`));
console.log(chalk.green(`[${pack.name}_${multiConfig.process.name}] 代理服务：${targetPath}`));
console.log(chalk.yellow(`[${pack.name}_${multiConfig.process.name}] 访问路径： http://localhost:${PORT}`));
